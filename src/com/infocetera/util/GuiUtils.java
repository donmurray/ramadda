...
public class GuiUtils {
    ...
    /** _more_ */
    public static final String[] BORDERS = { "empty", "raised", "sunken",
                                             "etched", "matte" };
    ...
    /** _more_ */
    public static final double[] DS_NNNNY = {
        0, 0, 0, 0, 0, 1
    };
    ...
    /** _more_ */
    public static final double[] DS_NNYNYNY = {
        0, 0, 1, 0, 1, 0, 1
    };
     ...
     * @return _more_
     */
    public static Container border(Component c, int borderType, int hGap,
                                   int vGap) {
        tmpInsets = new Insets(vGap, hGap, vGap, hGap);
        JPanel p = new BorderPanel(borderType,
    ...
    public static Vector parseCommands(String commands) {
        ...
        while (tok.hasMoreTokens()) {
            ...
            if ((index >= 0) && cmd.endsWith(")")) {
                ...
                //      System.err.println ("func:" + funcName +" params:" + params);
     ...
     * @return _more_
     */
    public static JPanel leftCenterRight(Component l, Component c,
                                         Component r) {
     ...
     * @return _more_
     */
    public static JPanel topCenterBottom(Component t, Component c,
                                         Component b) {
        JPanel cont = new JPanel(new BorderLayout());
        if (t != null) {
            cont.add("North", t);
     ...
     * @return _more_
     */
    public static Container doLayout(Component[] comps, int cols, double wx,
                                     double wy) {
        JPanel   c   = new JPanel();
        double[] wxa = { wx };
        double[] wya = { wy };
     ...
     * @return _more_
     */
    public static Container doLayout(Component[] comps, int cols,
     ...
     * @return _more_
     */
    public static Container doLayout(Container c, Component[] comps,
                                     int cols, double[] weightsX,
                                     double[] weightsY) {
     ...
     * @param height _more_
     */
    public static void paintBorder(Graphics g, int borderType, int width,
                                   int height) {
     ...
     * @param height _more_
     */
    public static void paintBorder(Graphics g, int borderType, Insets insets,
                                   Color[] colors, int width, int height) {
     ...
     */
    public static void paintBorder(Graphics g, int borderType, Insets insets,
                                   Color[] colors, int x, int y, int width,
                                   int height) {
        ...
        */
            ...
            if (insets.bottom != 0) {
                g.setColor(((colors[2] != null)
                            ? colors[2]
                            : Color.black));
                g.fillRect(x, y + height - insets.bottom, width,
                           insets.bottom);
            }

            if (insets.left != 0) {
                g.setColor(((colors[1] != null)
                            ? colors[1]
                            : Color.black));
     ...
     * @return _more_
     */
    public static Point rotatePoint(Point target, Point origin,
                                    double theta) {
        Point ret = rotatePoint(new Point(target.x - origin.x,
    ...
    public static Point rotatePoint(Point target, double theta) {
        return (new Point((int) (target.x * Math.cos(theta)
                                 - target.y * Math.sin(theta)
                                 + 0.5), (int) (target.x * Math.sin(theta)
                                     + target.y * Math.cos(theta) + 0.5)));
    }
     ...
     * @param where _more_
     */
    public static void drawStringAt(Graphics g, String t, int x, int y,
                                    int where) {
     ...
     * @param width _more_
     */
    public static void drawClippedString(Graphics g, String t, int x, int y,
                                         int width) {
     ...
     */
    public static void drawArc(Graphics g, int left, int top, int width,
                               int height, int startAngle, int deltaAngle,
                               int lineWidth) {
        ...
        for (int i = 0; i < lineWidth; i++) {
            ...
            if ((i + 1) < lineWidth) {
                g.drawArc(left, top, width - 1, height - 1, startAngle,
                          deltaAngle);
                g.drawArc(left + 1, top, width - 1, height - 1, startAngle,
                          deltaAngle);
                g.drawArc(left, top + 1, width - 1, height - 1, startAngle,
                          deltaAngle);
    ...
    public static void drawLine(Graphics g, int x1, int y1, int x2, int y2,
        ...
        if (lineWidth == 1) {
            ...
            double angle     = ((x1 == x2)
                                ? Math.PI
                                : Math.atan(deltaY / deltaX) + Math.PI / 2);
            ...
            int[] yCorners = { y1 - yOffset, y2 - yOffset, y2 + yOffset + 1,
                               y1 + yOffset + 1 };
    ...
    public static void drawRoundRect(Graphics g, int left, int top,
        ...
        for (int i = 0; i < lineWidth; i++) {
            ...
            if ((i + 1) < lineWidth) {
                g.drawRoundRect(left, top, width - 1, height - 1, arcWidth,
                                arcHeight);
    ...
    public static String replace(String source, String pattern, String to,
        ...
        String workSource = (caseSensitive
                             ? source
                             : source.toLowerCase());
