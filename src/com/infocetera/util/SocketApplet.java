...
public abstract class SocketApplet extends IfcApplet implements Runnable {
    ...
    /** _more_ */
    final static int BUFFER_SIZE = 1000000;
    ...
    private void readFromConnection(URLConnection connection, byte[] buffer) {
        try {
            ...
            //Keep going until we have read what we are expecting to read
            while (totalBytesRead < expectedBytes) {
                bytesRead = istream.read(buffer, totalBytesRead,
                                         buffer.length);
                if (bytesRead <= 0) {
                    break;
                } else {
            ...
            //Check if we are done, if so exit the loop
            if ( !isConnected) {
                return;
            }

            debug("readFromConnection: expectedBytes=" + expectedBytes
                  + " actually read:" + totalBytesRead);

            //Did we get everything we were expecting
            if (totalBytesRead < expectedBytes) {
                //Keep track of the errors. If after 10 errors then we quit this loop
                //and tell the user there was a problem
                if (++pollErrorCount > 10) {
                    errorMsg(
                        "The chat server does not seem to be responding");
                    pollErrorCount = 0;
                }
            } else {
                handleMessages(new String(buffer, 0, totalBytesRead));
                //Clear the error count because we were successful
                pollErrorCount = 0;
            }
    ...
    public void readSocket() {
        try {
            ...
            errorMsg(
                "An error has occurred trying to connect to:" + myHost
                + " on port: " + port
        ...
        try {
            ...
            while ((inputStream != null) && isConnected) {
                ...
                //Don't blow memory
                if (length > 1000000) {
                    throw new IllegalArgumentException("Bad length read:"
                            + length);
                }
                int total = 0;
                ...
                while (total < length) {
                    int howMany = inputStream.read(buffer, total,
                                      length - total);
                    if ( !isConnected) {
                        break;
                    }
            ...
            errorExc = exc;
            errorMsg =
                "read - The connection to the chat server has been lost";
        } catch (Throwable exc) {
            errorExc = exc;
        ...
        //If we are supposedly still connected then there was some error
        //Cleanup the sockets and show the error message
        if (isConnected) {
            closeSockets();
            if (errorMsg != null) {
    ...
    public void writePolling(String message) {
        try {
            ...
            if (lastMsgId != null) {
                urlString = GuiUtils.replace(urlString, "%LASTMESSAGEID%",
                                             lastMsgId);
            }
            ...
            if (pollPostTemplate != null) {
                ...
                postMessage = GuiUtils.replace(postMessage, "%MESSAGE%",
                        message);
            } else {
            ...
            //Now try to read any reply.  
            //The server might piggy back some messages from the write.
    ...
    public void writeSocket(String s) {
        ...
        try {
            ...
            errorMsg(
                "write - The connection to the chat server has been lost",
                exc);
        } catch (Throwable exc) {
