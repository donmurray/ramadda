...
import org.ramadda.geodata.data.PointDatabaseTypeHandler;
...
public class UshcnPointDatabaseTypeHandler extends PointDatabaseTypeHandler {
    ...
    protected void createDatabase(Request request, Entry entry, File dataFile,
        ...
        String dataContents = getStorageManager().readSystemResource(dataFile);
       ...
       metadata.add(new PointDataMetadata(tableName, colName,
                    metadata.size(), param, param, unit, 
                    PointDataMetadata.TYPE_DOUBLE));
    ...
    private void insertData(Entry entry, List<PointDataMetadata> metadata,
        ...
        for (int i = 0; i < dataLines.size(); i++) {
            ...
            if (line.length() < 72) {
                ...
                System.err.println("processing data for " + station.getName());
            ...
            if (daily) {
               ...
               System.err.println("got " + excp.getClass().getName() + " on line: " + line);
               continue;
            }
    ...
    private StationImpl parseStation(String line) throws Exception {
        ...
        double alt = Misc.parseDouble(line.substring(idx[4], idx[4]+7).trim());
