...
public class DbTypeHandler extends BlobTypeHandler {
    ...
    private Hashtable<String,Column> columnMap = new Hashtable<String,Column>();
    ...
    public DbTypeHandler(DbAdminHandler dbAdmin, Repository repository,     
        ...
        this.tableIcon = XmlUtil.getAttribute(tableNode, "icon", "/db/database.png");
        ...
        Element node = XmlUtil.create("column", root, new String[]{
                "name","contents",Column.ATTR_TYPE,"clob",Column.ATTR_SIZE,"256000",
                Column.ATTR_ADDTOFORM,"false",Column.ATTR_SHOWINHTML,"false"});
        List<Element> nodes = new ArrayList<Element>();
        nodes.add(node);
        super.init(nodes);

        setCategory("Database");
        tableHandler = new GenericTypeHandler(repository, "db_" + tableName, desc) {
    ...
    public void addToEntryNode(Entry entry, Element node) throws Exception {
        ...
        List<Object[]> valueList=readValues(Clause.eq(COL_ID, entry.getId()), "",-1);
    ...
    public void initializeEntryFromXml(Request request, Entry entry, Element node)
        ...
        String values = XmlUtil.getGrandChildText(node, TAG_DBVALUES,(String)null);
        if(values == null) return;
        List<Object[]> valueList=(List<Object[]>) xmlEncoder.toObject(new String(XmlUtil.decodeBase64(values)));
        if(valueList==null) throw new IllegalArgumentException("Could not read database value list");
        String sql = makeInsertOrUpdateSql(entry, null);
        PreparedStatement insertStmt = getRepository().getDatabaseManager().getPreparedStatement(sql);
        try {
    ...
    public void init(List<Element> columnNodes) throws Exception {
        ...
        for(Column column: columns) {
            ...
            if(descColumn == null && column.getType().equals(Column.TYPE_STRING) &&
               column.getRows()>1) {
                descColumn = column;
            }
            if(Misc.equals(column.getProperty("defaultsort"),"true")) {
                dfltSortColumn = column;
                dfltSortAsc = Misc.equals(column.getProperty("ascending"),"true");
            }


            if(doSums[cnt]) doSum = true;
            cnt++;
            ...
            if(column.isEnumeration() && Misc.equals(column.getProperty("iscategory"),"true")) {
        ...
        for(Column gridColumn: categoryColumns) {
            viewList.add(new TwoFacedObject(gridColumn.getLabel()+" " +"Category", VIEW_CATEGORY+gridColumn.getName()));
        }
    ...
    public String getInlineHtml(Request request, Entry entry) 
        ...
        List<Object[]> valueList=readValues(request, entry, Clause.eq(COL_ID, entry.getId()));
        StringBuffer sb = new StringBuffer();
        sb.append(HtmlUtil.cssLink(getRepository().getUrlBase()+"/db/dbstyle.css")) ;
    ...
    public Result getHtmlDisplay(Request request, Entry entry)
        ...
        if (!getRepository().getAccessManager().canAccessFile(request,
                entry)) {
            sb.append(getRepository().showDialogWarning(msg("You do not have permission to view database")));
        ...
        if(request.get(ARG_DB_SETPOS,false)) {
            if(!canEdit) throw new IllegalArgumentException("You cannot change the position");
            ...
            if(request.exists(ARG_DB_STICKYLABEL)) {
                ...
                Object[]values =  tableHandler.getValues(makeClause(entry, request.getString(ARG_DBID,"")));
                putProp("posx", values, new Integer(posx));
                putProp("posy", values, new Integer(posy));
                doStore(entry, values, false);
            }
            return new Result("", new StringBuffer("<contents>ok</contents>"), "text/xml");
        }


        if(request.exists(ARG_DB_EDITFORM)) {
            if(!canEdit) throw new AccessException("You cannot edit this database", request);
            return handleForm(request, entry, request.getString(ARG_DBID,(String)null),true);
        }
        ...
        if(request.exists(ARG_DB_EDIT) || request.exists(ARG_DB_COPY)) {
            if(!canEdit) throw new AccessException("You cannot edit this database", request);
            return handleNewOrEdit(request, entry,request.getString(ARG_DBID,(String)null));
        }
        ...
        if(request.exists(ARG_DB_ENTRY)) {
            return handleView(request, entry, request.getString(ARG_DBID,(String)null));
        }
    ...
    private void addViewHeader(Request request, Entry entry, StringBuffer sb, String view, int numValues, boolean fromSearch) throws Exception {

        addViewHeader(request, entry, sb, view, numValues, fromSearch, null);
    }

    private void addViewHeader(Request request, Entry entry, StringBuffer sb, String view, int numValues, boolean fromSearch, String extraLinks) throws Exception {


        List<String> headerToks = new ArrayList<String>();
        String baseUrl = HtmlUtil.url(request.url(getRepository().URL_ENTRY_SHOW),
                                      new String[]{
        ...
        if(view.equals(VIEW_TABLE)) {
            ...
            headerToks.add(HtmlUtil.href(baseUrl+"&" +ARG_DB_VIEW +"=" + VIEW_TABLE,
                                         msg("List")));
        }
        ...
        if(view.equals(VIEW_NEW)) {
            ...
            headerToks.add(HtmlUtil.href(baseUrl+"&" +ARG_DB_VIEW +"=" + VIEW_NEW,
                                         msg("New")));
        }
        ...
        if(view.equals(VIEW_SEARCH)) {
            ...
            headerToks.add(HtmlUtil.href(baseUrl+"&" +ARG_DB_VIEW +"=" + VIEW_SEARCH,
                                         msg("Search")));
        }
        ...
        */
        ...
        if(hasDate) {
            if(view.equals(VIEW_CALENDAR)) {
                ...
                headerToks.add(HtmlUtil.href(baseUrl+"&" +ARG_DB_VIEW +"=" + VIEW_CALENDAR,
                                             msg("Calendar")));
            }
        ...
        if(hasLocation) {
            if(view.equals(VIEW_MAP)) {
                ...
                headerToks.add(HtmlUtil.href(baseUrl+"&" +ARG_DB_VIEW +"=" + VIEW_MAP,
                                             msg("Map")));
            }
            if(view.equals(VIEW_KML)) {
                ...
                String kmlUrl = HtmlUtil.url(request.url(getRepository().URL_ENTRY_SHOW)+"/" + entry.getName()+".kml",
                                       new String[]{
                                           ARG_ENTRYID, entry.getId()});

                headerToks.add(HtmlUtil.href(kmlUrl+"&" +ARG_DB_VIEW +"=" + VIEW_KML,
                                             msg("KML")));
            }
        }
        ...
        if(hasNumber) {
            ...
            if(view.equals(VIEW_CHART)) {
                ...
                headerToks.add(HtmlUtil.href(baseUrl+"&" +ARG_DB_VIEW +"=" + VIEW_CHART,
                                             msg("Chart")));
            }
        }
        ...
        if(categoryColumns.size()>0) {
            String theColumn = request.getString(ARG_DB_COLUMN,categoryColumns.get(0).getName());
            for(Column column: categoryColumns) {
                String label =column.getLabel()+" - " + msg("Category View");
                if(view.equals(VIEW_CATEGORY+column.getName())) {
                    headerToks.add(HtmlUtil.b(label));
                } else {
                    headerToks.add(HtmlUtil.href(baseUrl+"&" +ARG_DB_VIEW +"=" + VIEW_CATEGORY+column.getName() +"&" + ARG_DB_COLUMN +"=" + column.getName(),
                                                 label));
                }
            }
        }
        ...
        if(headerToks.size()>1)  {
            sb.append(HtmlUtil.div(StringUtil.join("&nbsp;|&nbsp;", headerToks),HtmlUtil.cssClass("dbheader")));
        }
        ...
        if(extraLinks!=null) {
            sb.append(HtmlUtil.div(extraLinks,HtmlUtil.cssClass("dbheader")));
        }


        if(fromSearch) {
            sb.append(HtmlUtil.inset(HtmlUtil.makeShowHideBlock(msg("Search again"), getSearchForm(request, entry).toString(), false),10));
        }


        if(addNext) {
            if ((numValues > 0) && ((numValues == request.get(ARG_MAX, DB_MAX_ROWS)) || request.defined(ARG_SKIP))) {
                getRepository().getHtmlOutputHandler().showNext(request, numValues, sb);
    ...
    public Result handleList(Request request, Entry entry, String action)
        throws Exception {
        return handleList(request, entry, Clause.eq(COL_ID, entry.getId()), action, false);
    }
    


    public Result handleList(Request request, Entry entry, Clause clause, String action, boolean fromSearch)
        throws Exception {
        String view = request.getString(ARG_DB_VIEW,VIEW_TABLE);

        List<Object[]> valueList;

        if(dateColumns.size()>0 && request.defined(ARG_YEAR) && request.defined(ARG_MONTH)) {
            ...
            clause = Clause.and(clause, Clause.and(Clause.ge(dateColumns.get(0).getName(),date1),
        ...
        if(view.equals(VIEW_STICKYNOTES)) {
            return handleListStickyNotes(request, entry, valueList,fromSearch);
        }
    ...
    public void addToEntryForm(Request request, StringBuffer formBuffer,
        ...
        if(urlColumn!=null && entry!=null) {
            String baseUrl = getRepository().absoluteUrl(HtmlUtil.url(request.url(getRepository().URL_ENTRY_SHOW),
                                                                       new String[]{
                                                                           ARG_ENTRYID, entry.getId()}));
            String url = baseUrl+"&" +ARG_DB_VIEW +"=" + VIEW_NEW;
            String jsUrl = "javascript:document.location='" + url +"'+'&" + urlColumn.getFullName() +"='+" + "document.location";

            if(labelColumn!=null) {
                jsUrl = jsUrl +"+'&" + labelColumn.getFullName()+"='+document.title";
            }

            if(descColumn!=null) {
                String selected = "(window.getSelection? window.getSelection():document.getSelection?document.getSelection():document.selection?document.selection:'')";

                jsUrl = jsUrl +"+'&" + descColumn.getFullName()+"='+" +selected;
            }

            String href=HtmlUtil.href(jsUrl, " Add URL to " +entry.getName());
            formBuffer.append(HtmlUtil.row(
                                           HtmlUtil.colspan("Bookmark this link to add new items to the database: " + href,2)));
        }
        ...
        formBuffer.append(
                          HtmlUtil.row(
                                       HtmlUtil.colspan(
                                                        HtmlUtil.div(msg("Sticky Notes"), " class=\"formgroupheader\" "),
                                                        2)));


        String stickyLabelString = (String) props.get(PROP_STICKY_LABELS);
        if(stickyLabelString == null)stickyLabelString = "";
        formBuffer.append(HtmlUtil.formEntry(msg("Labels"),HtmlUtil.textArea(PROP_STICKY_LABELS,stickyLabelString,5,30)));
    ...
    public void initializeEntryFromForm(Request request, Entry entry, Group parent,
        ...
        props.put(PROP_STICKY_LABELS,StringUtil.join("\n",StringUtil.split(stickyLabelString,"\n",true,true)));
        setProperties(entry, props);
    ...
    public StringBuffer getSearchForm(Request request, Entry entry) throws Exception  {
        StringBuffer sb = new StringBuffer();
        ...
        sb.append(HtmlUtil.formEntry(msgLabel("Count"),
                                     HtmlUtil.input(ARG_MAX,request.get(ARG_MAX,100),HtmlUtil.SIZE_5)));
    ...
    public Result handleSearchForm(Request request, Entry entry) throws Exception  {
        StringBuffer sb = new StringBuffer();
    ...
    public Result handleSearch(Request request, Entry entry) throws Exception  {
        StringBuffer sb = new StringBuffer();
        List<Clause> where   = new ArrayList<Clause>();
    ...
    public Result handleDeleteConfirm(Request request, Entry entry) throws Exception  {
        ...
        try {
            ...
            getRepository().getDatabaseManager().closeAndReleaseConnection(statement);
        }
        
         
            

        String url = HtmlUtil.url(request.url(getRepository().URL_ENTRY_SHOW),
                                      new String[]{
                                      ARG_ENTRYID, entry.getId(),
                                      ARG_MESSAGE,"Entries deleted"
                                      });
    ...
    public Result handleDeleteAsk(Request request, Entry entry) throws Exception  {
        StringBuffer sb = new StringBuffer();
        List dbids = request.get(ARG_DBID_SELECTED,new ArrayList());
        
        if(dbids.size()==0) {
            sb.append(getRepository().showDialogWarning(msg("No entries were selected")));
        } else {
            String formUrl = request.url(getRepository().URL_ENTRY_SHOW);
            sb.append(HtmlUtil.form(formUrl));
            sb.append(HtmlUtil.hidden(ARG_ENTRYID, entry.getId()));
            ...
            for(Object dbid: dbids) {
                sb.append(HtmlUtil.hidden(ARG_DBID_SELECTED, dbid.toString()));
            }
            
            addViewHeader(request, entry, sb, "",0,false);
            sb.append(getRepository().showDialogQuestion(msg("Are you sure you want to delete the selected entries?"),
                                                         HtmlUtil.submit(msg("Yes"), ARG_DB_DELETECONFIRM) +HtmlUtil.space(2) +
                                                         HtmlUtil.submit(msg("Cancel"), ARG_DB_LIST)));
        }
    ...
    private void putProp(String prop, Object[]values, Object obj) throws Exception {
    ...
    private double getProp(String prop, Object[]values, double dflt) throws Exception {
        Hashtable props = getProps(values);
    ...
    private int getProp(String prop, Object[]values, int dflt) throws Exception {
        Hashtable props = getProps(values);
        if(props==null) return dflt;
    ...
    public Result handleNewOrEdit(Request request, Entry entry,String dbid) throws Exception  {
        if(request.exists(ARG_DB_COPY)) dbid = null;

        StringBuffer sb = new StringBuffer();
        List<String> colNames = tableHandler.getColumnNames();
        Object[]values = (dbid!=null? tableHandler.getValues(makeClause(entry, dbid)):
                           tableHandler.makeEntryValueArray());
        //The first entry is the db_id
        values[IDX_DBID] = (dbid==null?getRepository().getGUID():dbid);
    ...
    private void doStore(Entry entry, Object[] values, boolean isNew) throws Exception {
        String dbid = (String) values[IDX_DBID];
        String sql = makeInsertOrUpdateSql(entry,(isNew?null: dbid));
        PreparedStatement stmt = getRepository().getDatabaseManager().getPreparedStatement(sql);
        try {
            int stmtIdx = tableHandler.setStatement(entry, values, stmt, isNew);
            if(!isNew) {
    ...
    public String makeInsertOrUpdateSql(Entry entry, String dbid)     {
        if (dbid==null) {
            return SqlUtil.makeInsert(
                                      tableHandler.getTableName(), SqlUtil.comma(tableHandler.getColumnNames()),
                                      SqlUtil.getQuestionMarks(tableHandler.getColumnNames().size()));
        } else {
    ...
    public Result handleListEmail(Request request, Entry entry, List<Object[]> valueList) throws Exception {
    ...
    public Result handleListCsv(Request request, Entry entry, List<Object[]> valueList) throws Exception {
        StringBuffer sb = new StringBuffer();
    ...
    public Result handleListRss(Request request, Entry entry, List<Object[]> valueList) throws Exception {
        StringBuffer sb = new StringBuffer();
        ...
        sb.append(XmlUtil.tag(RssOutputHandler.TAG_RSS_TITLE, "", entry.getName()));
        for(int cnt=0;cnt<valueList.size();cnt++) {
            Object [] values = valueList.get(cnt);
            ...
            sb.append(XmlUtil.tag(RssOutputHandler.TAG_RSS_LINK, "", XmlUtil.getCdata(url)));


            sb.append(XmlUtil.tag(RssOutputHandler.TAG_RSS_GUID, "", XmlUtil.getCdata(url)));
    ...
    public String getEventJS(Request request,Entry entry, Object[]values, String rowId, String divId) {
        String xmlUrl =  getViewUrl(request,  entry,  ""+values[IDX_DBID])+"&result=xml";
        String event = HtmlUtil.onMouseOver(HtmlUtil.call("dbRowOver",HtmlUtil.squote(rowId))) +
            HtmlUtil.onMouseOut(HtmlUtil.call("dbRowOut", HtmlUtil.squote(rowId))) +
            HtmlUtil.onMouseClick(HtmlUtil.call("dbRowClick", "event," + HtmlUtil.squote(divId)+"," + HtmlUtil.squote(xmlUrl)));
        return event;
    }
    


    public Result handleListTable(Request request, Entry entry, List<Object[]> valueList, boolean fromSearch) 
        throws Exception {
        StringBuffer sb = new StringBuffer();
        String links = getHref(request, entry,VIEW_STICKYNOTES, msg("Sticky Notes"))  +"&nbsp;|&nbsp;" +
            getHref(request, entry,VIEW_RSS, msg("RSS"),"/"+entry.getName()+".rss")
            +"&nbsp;|&nbsp;" +
            getHref(request, entry,VIEW_CSV, msg("CSV"),"/"+entry.getName()+".csv");
        addViewHeader(request, entry, sb, VIEW_TABLE, valueList.size(),fromSearch, links);
    ...
    public void makeTable(Request request, Entry entry, List<Object[]> valueList, boolean fromSearch, StringBuffer sb,boolean doForm) 
        throws Exception {
        ...
        boolean asc = request.getString(ARG_DB_SORTDIR, (dfltSortAsc?"asc":"desc")).equals("asc");
        String sortBy =request.getString(ARG_DB_SORTBY, (dfltSortColumn ==null?"":dfltSortColumn.getName()));
        if(valueList.size()>0) {
            ...
            if(canEdit) {
                actions.add(new TwoFacedObject("Delete selected",ACTION_DELETE));
            }
            if(actions.size()>0) {
                if(doForm) {
            ...
            sb.append("<table class=\"dbtable\"  border=1 cellspacing=\"0\" cellpadding=\"2\" width=\"100%\">");
            sb.append("<tr>");
            sb.append("<td class=dbtableheader>&nbsp;</td>");
            for(int i=1;i<columns.size();i++) {
                if(!columns.get(i).getCanList()) continue;
                String label = columns.get(i).getLabel();
                String sortColumn =columns.get(i).getName();
                String extra;
                if(sortColumn.equals(sortBy)) {
                    if(asc) extra = " " + HtmlUtil.img(getRepository().iconUrl(ICON_UPDART));
                    else extra = " " +HtmlUtil.img(getRepository().iconUrl(ICON_DOWNDART));
                    asc = !asc;
                } else {
                    extra = " " +HtmlUtil.img(getRepository().iconUrl(ICON_BLANK), "", HtmlUtil.attr("width","10"));
                }
                
                String link= HtmlUtil.href(baseUrl+"&" + ARG_DB_SORTBY +"=" + sortColumn+"&" + ARG_DB_SORTDIR +(asc?
                                                                                                                "=asc":"=desc"),label) +extra;
                sb.append(HtmlUtil.col(link,HtmlUtil.cssClass("dbtableheader")));
        ...
        for(int cnt=0;cnt<valueList.size();cnt++) {
            ...
            sb.append("\n");
            sb.append(HtmlUtil.open(HtmlUtil.TAG_TR,HtmlUtil.attrs(HtmlUtil.ATTR_VALIGN,"top") +
                                    HtmlUtil.cssClass("dbrow") +
                                    HtmlUtil.id(rowId) +
                                    event));
            sb.append("<td width=\"10\" style=\"white-space:nowrap;\"><div id=\""  + divId+"\" >");
            ...
            String call = HtmlUtil.attr(
                                        ...
                                        HtmlUtil.call(
                                                      "checkboxClicked",
                                                      HtmlUtil.comma(
                                                                     "event", HtmlUtil.squote(ARG_DBID_SELECTED),
                                                                     HtmlUtil.squote(cbxId))));

            if(doForm) {
                sb.append(HtmlUtil.checkbox(ARG_DBID_SELECTED, dbid, false,HtmlUtil.id(cbxId) + call));
            }
            if(canEdit) {
                String editUrl =  getEditUrl(request,  entry,  dbid);
                sb.append(HtmlUtil.href(editUrl,HtmlUtil.img(getRepository().getUrlBase()+"/db/database_edit.png",msg("Edit entry"))));
            }

            String viewUrl =  getViewUrl(request,  entry,  dbid);
            sb.append(HtmlUtil.href(viewUrl,
            ...
            for(int i=1;i<columns.size();i++) {
                ...
                if(doSums[i]) {
                    ...
                    d = new Double(d.doubleValue()+((Double)values[column.getOffset()]).doubleValue());
        ...
        if(valueList.size()>0) {
            ...
            if(!fromSearch) {
                sb.append(HtmlUtil.br());
                sb.append(getRepository().showDialogNote(msg("No entries in: " + getTitle())));
            } else {
                sb.append(getRepository().showDialogNote(msg("Nothing found")));
            }
    ...
    public Result handleListMap(Request request, Entry entry, List<Object[]> valueList, boolean fromSearch) 
        throws Exception {
        boolean canEdit = getAccessManager().canEditEntry(request,  entry);
        StringBuffer sb = new StringBuffer();
        addViewHeader(request, entry, sb, VIEW_MAP, valueList.size(),fromSearch);
        Column theColumn = null;
        boolean bbox = true;
        ...
        for(Object[] values: valueList) {
            ...
            if(canEdit) {
                String editUrl = getEditUrl(request,  entry,  dbid);
                rightSide.append(HtmlUtil.href(editUrl,HtmlUtil.img(getRepository().getUrlBase()+"/db/database_edit.png",msg("Edit entry"))));
            }
            String viewUrl = getViewUrl(request,  entry,  dbid);
            rightSide.append(HtmlUtil.href(viewUrl,
                                           HtmlUtil.img(getRepository().getUrlBase()+"/db/database_go.png",msg("View entry"))));
            rightSide.append(" ");
            ...
            js.append("marker.setLabel(" + HtmlUtil.squote(getLabel(entry, values)) + ");\n");
        ...
        sb.append(
            "<table cellpadding=5 border=\"0\" width=\"100%\"><tr valign=\"top\"><td width=" + width +">");
    ...
    public Result handleListKml(Request request, Entry entry, List<Object[]> valueList, boolean fromSearch) 
        throws Exception {
        ...
        for(Object[] values: valueList) {
            ...
            String label = getLabel(entry, values);
            String viewUrl = getRepository().absoluteUrl(getViewUrl(request,  entry,  dbid));
            String href = HtmlUtil.href(viewUrl, label);
            StringBuffer desc = new StringBuffer(href+"<br>");
            getHtml(request, desc, entry, values);
            KmlUtil.placemark(folder, label, desc.toString(),  lat, lon, 0, null);
        }
    ...
    public Result handleListChart(Request request, Entry entry, List<Object[]> valueList, boolean fromSearch) 
        throws Exception {
        boolean canEdit = getAccessManager().canEditEntry(request,  entry);
        StringBuffer sb = new StringBuffer();
        addViewHeader(request, entry, sb, VIEW_CHART, valueList.size(),fromSearch);
        ...
        String fillerIcon  = getRepository().getUrlBase() + "/db/bluesquare.png";
        for(Column column: columns) {
            if(column.isNumeric()) {
                List data = new ArrayList();
                List labels = new ArrayList();
                boolean isPercentage  = column.getType().equals(Column.TYPE_PERCENTAGE);
                ...
                for(Object[]values: valueList) {
                    if(isPercentage) {
                        ...
                        int v = ((Integer)values[column.getOffset()]).intValue();
                        if(maxValue!=maxValue) maxValue =v;
                        maxValue = Math.max(v, maxValue);
                    } else {
                        double v = ((Double)values[column.getOffset()]).doubleValue();
                        if(maxValue!=maxValue) maxValue =v;
                        else maxValue = Math.max(v, maxValue);
                    }
                }
                ...
                sb.append(HtmlUtil.br());
                sb.append("\n");
                sb.append("<table xwidth=100% border=1 cellspacing=0 cellpadding=1>\n");
                for(Object[]values: valueList) {
                    double value;
                    if(isPercentage) {
                        ...
                        value = ((Integer)values[column.getOffset()]).intValue();
                    } else {
                        value =((Double)values[column.getOffset()]).doubleValue();
                    }
                    String url =  canEdit?getEditUrl(request,  entry, (String) values[IDX_DBID]):getViewUrl(request,  entry,   (String)values[IDX_DBID]);

                    String href = HtmlUtil.href(url,getLabel(entry, values));
                    String rowId  = "row_" + values[IDX_DBID];
                    String divId  = "div_" + values[IDX_DBID];
                    String event = getEventJS(request, entry, values,  rowId, divId);
                    sb.append("<tr " + HtmlUtil.id(rowId)+"> <td  width=10% " + HtmlUtil.id(divId) +  event+">" + HtmlUtil.space(2) + href+"&nbsp;</td>");
                    sb.append("<td align=right width=5%>" + value+"&nbsp;</td><td> ");
                    double percentage = value/maxValue;
                    sb.append("<img src=" + fillerIcon  +" height=10 width=" + (int)(8*percentage*100)+"></td>");
                    sb.append("</tr>\n");
                }
                sb.append("</table>\n");
    ...
    public Result handleListGrid(Request request, Entry entry, List<Object[]> valueList, boolean fromSearch) 
        throws Exception {
        ...
        if(gridColumn==null) throw new IllegalStateException("No grid columns defined");
        String links = getHref(request, entry,VIEW_CATEGORY+gridColumn.getName(), msg("Category View"));
        addViewHeader(request, entry, sb, VIEW_GRID+gridColumn.getName(), valueList.size(),fromSearch,links);
        ...
        if(gridColumn.getType().equals(Column.TYPE_ENUMERATION)) {
            ...
            enumValues = (List<String>)tableHandler.getEnumValues(gridColumn, entry);
        }
        ...
        for(String value: enumValues) {
            String searchUrl  =  HtmlUtil.url(request.url(getRepository().URL_ENTRY_SHOW),
                                         new String[]{
                                             ARG_ENTRYID, entry.getId(),
                                             ARG_DB_SEARCH, "true",
                                             key, value
                                         });
            sb.append(HtmlUtil.col("&nbsp;"+HtmlUtil.href(searchUrl,value),HtmlUtil.cssClass("dbtableheader")));
        }
        for(Object[]valuesArray: valueList) {
            sb.append("<tr>\n");
            String url =  canEdit?getEditUrl(request,  entry, (String) valuesArray[IDX_DBID]):getViewUrl(request,  entry,   (String)valuesArray[IDX_DBID]);

            String rowId = "row_" + valuesArray[IDX_DBID];
            String event = getEventJS(request, entry, valuesArray,  rowId, rowId);
            String href = HtmlUtil.href(url,getLabel(entry, valuesArray));
            //            href= HtmlUtil.span(href,HtmlUtil.cssClass("xdbcategoryrow")+);
            sb.append(HtmlUtil.col("&nbsp;"+href,HtmlUtil.id(rowId) + event+HtmlUtil.cssClass("dbcategoryrow")));
            String rowValue =   (String)valuesArray[gridColumn.getOffset()];
            for(String value: enumValues) {
                if(Misc.equals(value, rowValue)) {
                    sb.append(HtmlUtil.col("&nbsp;",HtmlUtil.cssClass("dbgridon")));
                } else {
                    sb.append(HtmlUtil.col("&nbsp;",HtmlUtil.cssClass("dbgridoff")));
                }
            }
        }
        sb.append("</table>");
    ...
    public String getHref(Request request,Entry entry, String view, String label) {
        return HtmlUtil.href(getUrl(request, entry, view), label);
    }

    public String getHref(Request request,Entry entry, String view, String label, String suffix) {
    ...
    public String getUrl(Request request,Entry entry, String view, String suffix) {
        return HtmlUtil.url(request.url(getRepository().URL_ENTRY_SHOW)+suffix,
                            new String[]{
                                ARG_ENTRYID, entry.getId(),
                                ARG_DB_VIEW, view});
    }

    public Result handleListCategory(Request request, Entry entry, List<Object[]> valueList, boolean fromSearch) 
        throws Exception {
        boolean canEdit = getAccessManager().canEditEntry(request,  entry);
        StringBuffer sb = new StringBuffer();
        String view = request.getString(ARG_DB_VIEW,"");
        ...
        if(gridColumn==null) throw new IllegalStateException("No grid columns defined");
        String links = getHref(request, entry,VIEW_GRID+gridColumn.getName(), msg("Grid View"));
        addViewHeader(request, entry, sb, VIEW_CATEGORY+gridColumn.getName(), valueList.size(),fromSearch, links);
        ...
        for(Object[]valuesArray: valueList) {
            ...
            String rowId = "row_" + valuesArray[IDX_DBID];
            String event = getEventJS(request, entry, valuesArray,  rowId, rowId);
            buffer.append(HtmlUtil.div(href,HtmlUtil.cssClass("dbcategoryrow")+HtmlUtil.id(rowId) + event));
            cnt++;
        }
        for(String rowValue: rowValues) {
            String block = HtmlUtil.makeShowHideBlock(rowValue, HtmlUtil.div(map.get(rowValue).toString(),HtmlUtil.cssClass("dbcategoryblock")),false);
            sb.append(block);
        }
    ...
    private boolean  isDataColumn(Column column) {
        if(column.getName().equals(COL_DBID) || column.getName().equals(COL_DBUSER)|| column.getName().equals(COL_DBCREATEDATE)|| column.getName().equals(COL_DBPROPS)) return false;
        return true;
    }



    public Result handleListMotionChart(Request request, Entry entry, List<Object[]> valueList, boolean fromSearch) 
        throws Exception {
        StringBuffer sb = new StringBuffer();
        addViewHeader(request, entry, sb, VIEW_CHART,valueList.size(),fromSearch);
        ...
        for(Column column: columns) {
            if(!isDataColumn(column)) continue;
            if(column.getType().equals(Column.TYPE_DATE)) {
                if(dateColumn==null)
                    dateColumn = column;
                continue;
        ...
        for(Object[] values: valueList) {
            columnCnt=0;
            String label = getLabel(entry, values);
            sb.append("data.setValue(" + row + ", " + columnCnt +"," + HtmlUtil.squote(label)+");\n");
            columnCnt++;
            ...
            sb.append("data.setValue(" + row + ", " + columnCnt +", theDate);\n");
            columnCnt++;
            ...
            for(Column column: columns) {
                ...
                if (column.isNumeric()) {
                    sb.append("data.setValue(" + row + ", " + columnCnt +"," + values[column.getOffset()]+");\n");
                    columnCnt++;
                } else if (column.isString()) {
                    sb.append("data.setValue(" + row + ", " + columnCnt +"," + HtmlUtil.squote(""+values[column.getOffset()])+");\n");
                    columnCnt++;
                } else if(column.getType().equals(Column.TYPE_DATE)) {
                    columnCnt++;
                }
    ...
    public Result handleListTimeline(Request request, Entry entry, List<Object[]> valueList, boolean fromSearch) 
        throws Exception {
        Column theColumn = null;
        for(Column column: columns) {
            if(column.getType().equals(Column.TYPE_DATE)) {
        ...
        StringBuffer sb = new StringBuffer();
        addViewHeader(request, entry, sb, VIEW_TIMELINE,valueList.size(),fromSearch);
    ...
    public Result handleListCalendar(Request request, Entry entry, List<Object[]> valueList, boolean fromSearch) 
        throws Exception {
        boolean canEdit = getAccessManager().canEditEntry(request,  entry);
        StringBuffer sb = new StringBuffer();
        String links = getHref(request, entry,VIEW_TIMELINE, msg("Timeline")) +"&nbsp;|&nbsp;" +
            getHref(request, entry,VIEW_ICAL, msg("ICAL"));
        addViewHeader(request, entry, sb, VIEW_CALENDAR,valueList.size(),fromSearch, links);



        CalendarOutputHandler calendarOutputHandler = (CalendarOutputHandler) getRepository().getOutputHandler(
                                                                                                               CalendarOutputHandler.OUTPUT_CALENDAR);

        List<CalendarOutputHandler.CalendarEntry> calEntries = new ArrayList<CalendarOutputHandler.CalendarEntry>();
        ...
        for (Object[]values: valueList) {
            ...
            if(canEdit) {
                String editUrl =  getEditUrl(request,  entry,  dbid);
                href = HtmlUtil.href(editUrl,HtmlUtil.img(getRepository().getUrlBase()+"/db/database_edit.png",msg("Edit entry"))) +" " + href;
            }
            //            html.append(href);
            String rowId = "row_" + values[IDX_DBID];
            String event = getEventJS(request, entry, values,  rowId, rowId);
            href = HtmlUtil.div(href,HtmlUtil.cssClass("dbcategoryrow")+HtmlUtil.id(rowId) + event);
        //            getHtml(request, html, entry, values);
            String block = HtmlUtil.makeShowHideBlock(href, html.toString(),false);
            calEntries.add(new CalendarOutputHandler.CalendarEntry(date, href, href));
        }
    ...
    public Result handleListStickyNotes(Request request, Entry entry, List<Object[]> valueList, boolean fromSearch) 
        throws Exception {

        Hashtable entryProps =  getProperties(entry);
        String stickyLabelString = (String) entryProps.get(PROP_STICKY_LABELS);


        boolean canEdit = getAccessManager().canEditEntry(request,  entry);
        StringBuffer sb = new StringBuffer();
        StringBuffer js = new StringBuffer();
        addViewHeader(request, entry, sb, VIEW_STICKYNOTES,valueList.size(),fromSearch);
        ...
        int cnt = 0;
        int poscnt = 0;
        sb.append(HtmlUtil.importJS(getRepository().fileUrl("/db/dom-drag.js")));
        for (Object[]values: valueList) {
            Hashtable props = getProps(values);
            String dbid = (String)values[IDX_DBID];
            String url = getViewUrl(request,  entry,  dbid);
            ...
            String info = getHtml(request,  entry,  dbid,  columns,  values);
            String contents = href+HtmlUtil.makeShowHideBlock("...",info,false);
            sb.append(HtmlUtil.div(contents,  HtmlUtil.cssClass("dbstickynote") + HtmlUtil.id(dbid) +
            ...
            if(canEdit) {
                String posUrl =  HtmlUtil.url(request.url(getRepository().URL_ENTRY_SHOW),
                                                     new String[]{
                                                         ARG_ENTRYID, entry.getId(),
                                                         ARG_DBID, dbid,
                                                         ARG_DB_SETPOS,
                ...
                js.append("draggableDiv.onDragEnd  = function(x,y){stickyDragEnd(" + jsid +"," + HtmlUtil.squote(posUrl) +");}\n");
            }
        }

        if(stickyLabelString!=null) {
            int labelCnt = 0;
            for(String label: StringUtil.split(stickyLabelString,"\n", true,true)) {
                String id = "label_" + label;
                int top = 100;
                int left = 150+labelCnt*30;
                String posx = (String) entryProps.get(PROP_STICKY_POSX+"." + label);
                String posy = (String) entryProps.get(PROP_STICKY_POSY+"." + label);
                ...
                String text = label; 
                sb.append(HtmlUtil.div(text,  HtmlUtil.cssClass("dbstickylabel") + HtmlUtil.id(id) +
                                       " style=\"top:" + top +"px;  left:" + left +"px;\""));           

                js.append("var draggableDiv = document.getElementById(" + HtmlUtil.squote(id)  +");\n");
                js.append("Drag.init(draggableDiv);\n");
                if(canEdit) {
                    String posUrl =  HtmlUtil.url(request.url(getRepository().URL_ENTRY_SHOW),
                                                     new String[]{
                    ...
                    js.append("draggableDiv.onDragEnd  = function(x,y){stickyDragEnd(" + HtmlUtil.squote(id) +"," + HtmlUtil.squote(posUrl) +");}\n");
                }
            }
        }
    ...
    public Result handleListIcal(Request request, Entry entry, List<Object[]> valueList, boolean fromSearch) 
        throws Exception {
        SimpleDateFormat sdf =  RepositoryUtil.makeDateFormat("yyyyMMdd'T'HHmmss");
        StringBuffer sb = new StringBuffer();
        ...
        for (Object[]values: valueList) {
            ...
            Date date2 = (Date) values[dateColumns.size()>1?dateColumns.get(1).getOffset():dateColumns.get(0).getOffset()];
    ...
    public List<Object[]> readValues(Request request, Entry entry, Clause clause)
        throws Exception {
        if(request.exists(ARG_DBIDS)) {
            ...
            for(String id: StringUtil.split(ids,",",true,true)) {
                result.addAll(readValues(request, entry, makeClause(entry, id)));
            }
        ...
        if(dfltSortColumn!=null && !request.defined(ARG_DB_SORTBY)) {
            ...
            request.put(ARG_DB_SORTDIR,dfltSortAsc?"asc":"desc");
        }
        ...
        if(request.defined(ARG_DB_SORTBY)) {
            ...
            if(column!=null) {
                ...
                boolean asc = request.getString(ARG_DB_SORTDIR, "asc").equals("asc");
                extra += " order by " + by + (asc?" asc ":" desc ");
            }
        }
        int max = request.get(ARG_MAX, 100);
        int skip = request.get(ARG_SKIP, 0);
        extra += getDatabaseManager().getLimitString(skip,
                                                     max);
        return readValues(clause, extra, max);
    }


    private List<Object[]> readValues(Clause clause, String extra, int max) throws Exception {
        List<Object[]> result = new ArrayList<Object[]>();
    ...
    public void deleteEntry(Request request, Statement statement, String id)
        ...
        String query = SqlUtil.makeDelete(tableHandler.getTableName(), COL_ID,
                                          SqlUtil.quote(id));
    ...
    public Result handleForm(Request request, Entry entry, String dbid, boolean forEdit)
        throws Exception {

        List<String> colNames = tableHandler.getColumnNames();
        StringBuffer sb = new StringBuffer();
        addViewHeader(request, entry, sb, (dbid==null?VIEW_NEW:""),0,false);
        ...
        if(forEdit)  {
            if(dbid==null) {
                buttons.append(HtmlUtil.submit(msg("Create entry"), ARG_DB_CREATE));
            } else {
                buttons.append(HtmlUtil.submit(msg("Edit entry"), ARG_DB_EDIT));
                buttons.append(HtmlUtil.submit(msg("Copy entry"), ARG_DB_COPY));
                buttons.append(HtmlUtil.submit(msg("Delete entry"), ARG_DB_DELETE));
            }
        }
        ...
        tableHandler.addColumnsToEntryForm(request, 
                                           formBuffer, entry, values);
        ...
        if(false && forEdit && dbid==null) {
            ...
            bulkButtons.append(HtmlUtil.submit(msg("Create entries"), ARG_DB_CREATE));
            ...
            for(Column column: columns) {
                if(cnt>0)  {
                    colIds.add(new TwoFacedObject(column.getLabel(), column.getName()));
                }
                cnt++;
            }
            cnt =0;
            for(Column column: columns) {
                if(cnt>0)  {
                    bulk.append(HtmlUtil.select(ARG_DB_BULKCOL+cnt, colIds,columns.get(cnt).getName()));
                }
                cnt++;
            }
            bulk.append(HtmlUtil.br());
            ...
            String contents = HtmlUtil.makeTabs(Misc.newList(msg("Form"),
                                                             msg("Bulk Create")), 
                                                Misc.newList(formBuffer.toString(),
    ...
    private void getHtml(Request request, StringBuffer sb, Entry entry, Object[]values) throws Exception {
        sb.append(HtmlUtil.formTable());
        for (Column column : columns) {
            if(!isDataColumn(column)) continue;
    ...
    private String getLabel(Entry entry, Object[] values) 
        throws Exception {
        String lbl = getLabelInner(entry, values);
        if(lbl==null || lbl.trim().length()==0) lbl = "---";
        return lbl;
    }
    ...
    private String getHtml(Request request, Entry entry, String dbid, List<Column> columns, Object[] values)
        throws Exception {
        StringBuffer sb = new StringBuffer();
        sb.append(HtmlUtil.formTable());
        int valueIdx = 0;
