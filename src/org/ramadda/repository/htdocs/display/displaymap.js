...
function RamaddaMapDisplay(displayManager, id, properties) {
	...
	RamaddaUtil.defineMembers(this, {
		...
		addMapLayer : function(source, props) {
			...
			this.map.addWMSLayer(entry.getName(), entry["column.base_url"],
					entry["column.layer_name"], false);
		},
		...
		handleEventEntrySelection : function(source, args) {
			...
			if (selected) {
				...
				// console.log("set selection marker:" +entry.getLongitude()+" "
				// + entry.getLatitude());
				this.map.setSelectionMarker(entry.getLongitude(), entry
						.getLatitude(), true, args.zoom);
